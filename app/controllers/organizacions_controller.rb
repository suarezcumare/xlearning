...
class OrganizacionsController < ApplicationController
  ...
  def edit_perfil
   ...
   def save_perfil
         ...
         @organizacion.update(organizacion_params)
          ...
          if ! params[:usuario][:perfil_attributes].nil?
  ...
  def save_rol
    ...
    @usuarioRol = UsuarioRol.where("usuario_id = ? and rol_id = ?", params[:idusuario], params[:idrol])

     @usuarioRol.each do |usr|
         @usuarioRol = usr
     end 
    ...
    if  @usuarioRol.count <= 0
           ...
           @usuarioRol.rol_id = params[:idrol]
           @usuarioRol.usuario_id = params[:idusuario]
           @usuarioRol.save  
               
    end

    render "organizacions/usuarios"
  end

  def aceptar
      
      @grupo = Grupo.where("id = ? and usuario_id = ?", params[:grupo],  params[:idusuario])
