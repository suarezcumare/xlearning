...
{if(!RG.cache[id]){RG.cache[id]={};RG.cache[id].object=obj;RG.cache[id].canvas=jQuery('<canvas></canvas>').attr({width:obj.canvas.width,height:obj.canvas.height,id:'background_cached_canvas'+obj.canvas.id}).get(0);if(typeof G_vmlCanvasManager==='object'&&G_vmlCanvasManager.initElement){G_vmlCanvasManager.initElement(RG.cache[id].canvas);}
